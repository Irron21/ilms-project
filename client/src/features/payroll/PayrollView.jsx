...
function PayrollView() {
  ...
  const availableMonths = [
    ...new Set(filteredPeriods.map((p) => p.periodName.split(" ")[0])),
  ];

  // Get periods for the selected month
  const periodsInMonth = filteredPeriods.filter((p) =>
    p.periodName.startsWith(selectedMonth),
  );
  ...
  useEffect(() => {
    if (selectedPeriod && periods.length > 0) {
      const p = periods.find(
        (item) => item.periodID === Number(selectedPeriod),
      );
      if (p) {
  ...
  const confirmLockPeriod = () => {
    setFeedbackModal({
      ...
      subMessage:
        "No more adjustments or payments can be made. This action is final.",
  ...
  const handleSelectAllPeriods = () => {
    const periodsInExportYear = periods.filter((p) =>
      p.periodName.includes(selectedExportYear),
    );
    const allInYearSelected = periodsInExportYear.every((p) =>
      selectedExportPeriods.includes(p.periodID),
    );

    if (allInYearSelected) {
      // Deselect all for this year
      const yearIds = periodsInExportYear.map((p) => p.periodID);
      setSelectedExportPeriods((prev) =>
      ...
      const yearIds = periodsInExportYear.map((p) => p.periodID);
      setSelectedExportPeriods((prev) => [...new Set([...prev, ...yearIds])]);
    }
  };
  ...
  const handleExport = async () => {
    ...
    try {
      const selectedKeys = columnConfig
        .filter((c) => c.checked)
        .map((c) => c.key);
      ...
      const fileName =
        selectedExportPeriods.length === 1
  ...
  const fetchPayrollSummary = async (id) => {
    ...
    try {
      ...
      // Calc Stats
      const totalSal = res.data.reduce(
        (sum, row) => sum + Number(row.netSalary),
        0,
      );
      const totalAllow = res.data.reduce(
        (sum, row) => sum + Number(row.totalAllowance),
        0,
      );
      setStats({
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      alert(
        "Failed to generate payroll. Please check if the server is running.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const confirmGeneratePeriods = () => {
    setFeedbackModal({
      ...
      message:
        "This will automatically generate payroll periods for the next 12 months.",
      ...
      <div className="payroll-header">
        <div className="payroll-controls">
          <div className="filter-group-bordered">
            <div style={{ display: "flex", flexDirection: "column" }}>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  const firstInYear = periods.find((p) =>
                    p.periodName.includes(e.target.value),
                  );
            ...
            <div
              style={{ width: "1px", height: "25px", background: "#eee" }}
            ></div>
            <div style={{ display: "flex", flexDirection: "column" }}>
              <div
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const firstPeriod = filteredPeriods.find((p) =>
                      p.periodName.startsWith(newMonth),
                    );
          ...
          <div
            style={{ width: "1px", height: "35px", background: "#eee" }}
          ></div>
          <button
          ...
          <button
            ...
            style={{
              ...
              cursor:
                !selectedPeriod || isPeriodLocked() ? "not-allowed" : "pointer",
            }}
            title={
              !selectedPeriod
              ...
              <tbody>
                {paginatedData.map((row) => (
                  <tr key={row.userID}>
                    ...
                    <td>
                      <span
                        className={`role-badge ${row.role === "Driver" ? "role-driver" : "role-helper"}`}
                      >
                        {row.role}
                      </span>
                    </td>
                    ...
                    <td
                      className="text-right net-pay-cell"
                    ...
                    <td className="text-right">{formatMoney(row.totalPaid)}</td>
                    <td style={{ textAlign: "center" }}>
                      ...
                      Number(row.totalPaid) === Number(row.netSalary) &&
                        ...
                        <span
                          ...
                          style={{ background: "#f5f6f7", color: "#7f8c8d" }}
                        >
                          PENDING
                        </span>
                      )}
                      ...
                      >
                        ...
                        {(Number(row.netSalary) > 0 ||
                          ...
                          <button
                            ...
                            title={
                              isPeriodLocked()
                                ? "View Payments (Period Closed)"
                                : Number(row.totalPaid) < Number(row.netSalary)
                                  ? "Pay Remaining"
                                  : "View Payments"
                            }
                            style={{
                              color: isPeriodLocked() ? "#2d3436" : "#27ae60",
                            }}
                          >
                            <Icons.Cash size={18} />
                          </button>
          ...
          >
            ...
            <div className="modal-form-body">
              ...
              >
                ...
                >
                  {periods
                    .filter((p) => p.periodName.includes(selectedExportYear))
                    .every((p) => selectedExportPeriods.includes(p.periodID))
                    ? "Deselect All"
                    : "Select All"}
                </button>
              </div>
              ...
              <div className="period-multiselect-container">
                {periods.filter((p) =>
                    ...
                    .map((p) => (
                      <label key={p.periodID} className="period-checkbox-item">
                        ...
                        <span
                          style={{
                            fontSize: "11px",
                            color: p.status === "OPEN" ? "#27ae60" : "#95a5a6",
                            marginLeft: "auto",
                          }}
                        >
                          {p.status}
                  ...
                  >
                    No periods found for {selectedExportYear}.
                  </div>
                )}
              </div>
              ...
              >
                ...
                >
                  {columnConfig.every((col) => col.checked)
                    ? "Deselect All"
                    : "Select All"}
                </button>
              </div>
...
const EmptyState = () => (
  ...
  >
    ...
    <p style={{ fontSize: "13px" }}>
      Select a period above and click "Generate" to see the suggestion.
    </p>
  </div>
);
